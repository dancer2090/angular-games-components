...
module.exports = function(options) {
  ...
  const mobileAssets = new CopyWebpackPlugin([
      { from: 'src/assets/css', to: 'assets/css' },
      { from: 'src/assets/jquery', to: 'assets/jquery' },
      { from: 'src/assets/script', to: 'assets/script' },
      { from: 'src/libs/videojs', to: 'libs/videojs' }
    ],    
    isProd ? { ignore: ['mock-data/**/*'] } : undefined
  )

  const webAssets = new CopyWebpackPlugin([
    { from: 'src/assets/css', to: 'assets/css' },
    { from: 'src/assets/product_data_11.json', to: 'assets' },
    { from: 'src/assets/icon/outlined', to: 'assets/icon/outlined' }
    ],
    isProd ? { ignore: ['mock-data/**/*'] } : undefined
  )  
  ...
  return {
    ...
    module: {
      rules: [
         ...
         */
        {
          ...
          use: "file-loader?name=[name].[ext]&publicPath=" + METADATA.cdnLocation + "assets/icon/&outputPath=/assets/icon/"
        },
        ...
        */
        {
          test: /\.(eot|woff2?|ttf)([\?]?.*)$/,
          use: "file-loader?name=[name].[ext]&publicPath=" + METADATA.baseUrl + "assets/font/&outputPath=assets/font/"
        }
      ]
    },
    ...
    plugins: [
      ...
      (process.env.PLATFORM === 'IPAD' || process.env.PLATFORM === 'ANDROID') ? mobileAssets : webAssets,
      // new CopyWebpackPlugin(
      //   [{ from: 'src/assets', to: 'assets' }, { from: 'src/meta' }],
      //   isProd ? { ignore: ['mock-data/**/*', 'audio/**/*', 'programInfoImages/**/*', 'captions/**/*', 'font/**/*', 'jquery/**/*', 'script/**/*', 'icon/**/*', 'video/**/*', 'captions/**/*'] } : undefined
      // ),
