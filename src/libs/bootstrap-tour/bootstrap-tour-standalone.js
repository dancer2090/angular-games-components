...
(function(window, factory) {
  ...
  Tour = (function() {
    ...
    Tour.prototype._scrollIntoView = function(step, callback) {
      var $element, $window, counter, height, offsetTop, scrollTop, windowHeight, container, currentScroll;
      $element = $(step.element);
      if (!$element.length) {
        return callback();
      }
      ...
      switch (step.placement) {
        case 'top':
          scrollTop = Math.max(0, currentScroll + offsetTop - (windowHeight / 2));
          break;
        case 'left':
        case 'right':
          scrollTop = Math.max(0, currentScroll + (offsetTop + height / 2) - (windowHeight / 2));
          break;
        case 'bottom':
          scrollTop = Math.max(0, currentScroll + (offsetTop + height) - (windowHeight / 2));
      }
	  ...
	  return $(container).stop(true, true).animate({
