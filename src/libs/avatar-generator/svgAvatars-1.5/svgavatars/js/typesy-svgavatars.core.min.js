...
K="up down left right scaledown scaleup tiltleft tiltright".split(" "),Z="random reset save share gravatar download".split(" "),aa=["twitter","pinterest"],W=["#19A6CA","#CB2028"],O={backs:"backs",face:"faceshape",eyes:"eyesfront",hair:"hair",clothes:"clothes"},l=1,H=!1,p="faceshape",U,R=!1,q,r;h();for(var m=0;m<z.length;m++) if(av.indexOf(z[m])==-1) a("#svga-blocks").append('<div id="svga-blocks-'+z[m]+'" class="svga-blocks disabled" data-blockname="'+z[m]+'">'+f.blockTitles[z[m]]+'<img class="svga-blocks-img-locked" src="'+window.my.namespace.getLevelOfFeature(z[m],true)+'"/><span class="svga-blocks-tooltiptext '+z[m]+'">You need to reach '+window.my.namespace.getLevelOfFeature(z[m])+' level to unlock this feature</span></div>'); else a("#svga-blocks").append('<div id="svga-blocks-'+z[m]+'" class="svga-blocks" data-blockname="'+z[m]+'">'+f.blockTitles[z[m]]+"</div>");a(".svga-blocks:last").addClass("svga-last");
...
a("#svga-ios").hide();a("#svga-share-block").hide();a("#svga-gravatar-message").fadeOut("fast");a("#svga-gravataravatar").trigger("click")});a("#svga-saveavatar").on("click", window.saveAvatar=function(){/*a("#svga-work-overlay").fadeIn("fast");*/a("#svga-canvas").attr({width:c.savingSize,height:c.savingSize});q&&q.hide();r&&r.hide();var b=a("#svga-svgmain").html();b='<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="'+c.savingSize+'px" height="'+c.savingSize+'px" viewBox="0 0 200 200" style="overflow:hidden!important;">'+
...
"svg"===c.saveFileFormat&&(e=b,h="svgA"+(Math.random()+"").replace("0.","")+".svg"),a("#svga-canvas").attr({width:200,height:200}),window.my.namespace.uploadAvatar(e); /*a.ajax({url:c.pathToFolder+"php/save-ready-avatar.php",type:"post",dataType:"text",data:{imgdata:e,filename:h},cache:!1}).done(function(b){"saved"===b?(a("#svga-message-text").html(f.alertSuccess).removeClass("svga-error"),a("#svga-work-overlay").fadeIn("fast")):"error_wordpress"===b?(a("#svga-work-overlay").fadeIn("fast"),a("#svga-message-text").html(f.alertWordpressFail).addClass("svga-error")):
...
a("#svga-start-boys").on("click",function(){a("#svga-gender").hide();a.ajax({url:c.pathToFolder+"json/svgavatars-male-data.json?",dataType:"json",cache:!0,global:!1}).done(function(a){L("boys",a);window.my.namespace.enableSaveButton()}).fail(function(){a("#svga-message-text").html(f.alertJsonError).addClass("svga-error");a("#svga-loader").hide();a(".svga-close").hide();a("#svga-work-overlay").fadeIn("fast");a("#svga-message").fadeIn("fast")})}),a("#svga-start-girls").on("click",function(){a("#svga-gender").hide();a.ajax({url:c.pathToFolder+
"json/svgavatars-female-data.json?",dataType:"json",cache:!0,global:!1}).done(function(a){L("girls",a);window.my.namespace.enableSaveButton()}).fail(function(){a("#svga-message-text").html(f.alertJsonError).addClass("svga-error");a("#svga-loader").hide();a(".svga-close").hide();a("#svga-work-overlay").fadeIn("fast");a("#svga-message").fadeIn("fast")})}),"boysonly"===c.showGender?a("#svga-start-boys").trigger("click"):"girlsonly"===c.showGender&&a("#svga-start-girls").trigger("click"),a(window).on("resize orientationchange",
function(){da()}),a(".scrollbar").scrollbar({showArrows:!1,ignoreMobile:!1}))});//"json/svgavatars-male-data.json?ver="+c.version,dataType:"json"
